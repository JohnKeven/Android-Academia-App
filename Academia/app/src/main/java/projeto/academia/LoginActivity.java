...
public class LoginActivity extends AppCompatActivity implements LoaderCallbacks<Cursor> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        context = this;
        iUsuario = new UsuarioDAO(context);
        iCidade = new CidadeDAO(context);
        iFaixa = new FaixaDAO(context);

        this.testarInicializacao();
    }

    public void onClickEntrar(View view) {
        String email = mEmailView.getText().toString();
        String senha = mPasswordView.getText().toString();
        boolean isValid = iUsuario.pesquisarPorLogin(email, senha);

        if (isValid) {
            startActivity(new Intent(context, MenuActivity.class));
        } else {
            Toast.makeText(context, "Usuário ou senha está incorreto!", Toast.LENGTH_LONG).show();
        }
    }

    private void testarInicializacao() {
        if (iUsuario.retornarTodos().isEmpty()) {
            Usuario usuario = new Usuario("Admin", "admin", "admin");
            iUsuario.inserir(usuario);
        }

        if (iCidade.retornarTodos().isEmpty()) {
            Cidade cidade;
            cidade = new Cidade("Lagarto");
            iCidade.inserir(cidade);
            cidade = new Cidade("Poço Verde");
            iCidade.inserir(cidade);
        }

        if (iFaixa.retornarTodas().isEmpty()) {
            Faixa faixa;
            faixa = new Faixa("Sem Faixa");
            iFaixa.inserir(faixa);
            faixa = new Faixa("Branca");
            iFaixa.inserir(faixa);
        }
    }
